...
class MyCOSinverse : public mpFY {
 ...
 public:
  MyCOSinverse(double freq, double amp) : mpFY(wxT("g(y) = COS(y)"), mpALIGN_BOTTOM) {
    m_freq = freq;
    m_amp = amp;
    m_drawOutsideMargins = false;
  }
...
class MyLissajoux : public mpFXY {
 ...
 public:
  MyLissajoux(double rad) : mpFXY(wxT("Lissajoux")) {
    m_rad = rad;
    m_idx = 0;
    m_drawOutsideMargins = false;
  }
...
MyFrame::MyFrame()
  ...
  m_plot->AddLayer(nfo = new mpInfoCoords(wxRect(80, 20, 10, 10), wxTRANSPARENT_BRUSH));
  nfo->SetVisible(false);
  mpInfoLegend *leg;
  m_plot->AddLayer(leg = new mpInfoLegend(wxRect(200, 20, 40, 40), wxTRANSPARENT_BRUSH));
  leg->SetVisible(true);

  // m_plot->EnableCoordTooltip(true);
  l->SetPen(wxPen(*wxRED, 5, wxPENSTYLE_SOLID));
...
void MyFrame::OnSaveScreenshot(wxCommandEvent &event) {
  wxFileDialog fileDialog(this, _("Save a screenshot"), wxT(""), wxT(""),
                          wxT("BMP image (*.bmp) | *.bmp|JPEG image (*.jpg) | *.jpeg;*.jpg|PNG image (*.png) | *.png"),
                          wxFD_SAVE);
...
void MyFrame::OnToggleLissajoux(wxCommandEvent &event) { m_plot->SetLayerVisible(wxT("Lissajoux"), event.IsChecked()); }

void MyFrame::OnToggleSine(wxCommandEvent &event) { m_plot->SetLayerVisible(wxT("f(x) = SIN(x)"), event.IsChecked()); }
